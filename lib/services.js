 ...
 */
module.exports = function generateServices(app, ngModuleName, apiUrl, framework) {
  ngModuleName = ngModuleName || 'lbServices';
  apiUrl = apiUrl || '/';
  framework = framework || 'angular';

  var models = describeModels(app);
  var templateName = templateName = './services.template.' + framework + '.ejs';

  try {
    fs.accessSync(require.resolve(templateName), fs.F_OK);
  }
  catch(e) {
    console.error('\033[31mTemplate for "%s" not found\033[0m', framework);
    process.exit(1);
  }
